...
def node_enumeration(neighbor_info, cmp_locations, resistor_dimensions):
    ...
    for node in neighbor_info:
        ...
        for neighbor in neighbors:
            ...
            if nodes[i] == 3:
                cv2.circle(loop_detection_img, (int(node_x), int(node_y)), 5, (255, 255, 0), -1)
                cv2.putText(loop_detection_img, str(nodes[i]), (int(node_x)-20, int(node_y)+20), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (255, 255, 0), 1, cv2.LINE_AA) 
